...
- **BUG-001** sd **BUG-010**: lihat `dev-log.md` & `CHANGELOG.md`. Highlight: jangan pakai `useActionState` (React 19), jangan bikin folder `@/src/*` baru (BUG-002 — folder lama sudah dihapus, semua kode di `app/`), jangan `window.confirm()`, jangan single-frame face embedding, jangan capture embedding di pose `turnRight`.
