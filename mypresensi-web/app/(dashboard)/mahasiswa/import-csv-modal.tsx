'use client'
// app/(dashboard)/mahasiswa/import-csv-modal.tsx
// Modal untuk import mahasiswa via CSV.

import { useState } from 'react'
import { useFormState, useFormStatus } from 'react-dom'
import { Upload, X, FileSpreadsheet } from 'lucide-react'
import { importStudentsCSVAction, StudentFormState } from '@/lib/actions/students'
import { swal } from '@/lib/swal'

const initialState: StudentFormState & { imported?: number; skipped?: number } = {
  error: null,
  success: false,
}
...
export default function ImportCSVModal() {
  ...
  const [state, formAction] = useFormState(importStudentsCSVAction, initialState)
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <h3 className="text-base font-bold font-heading text-text-primary flex items-center gap-2">
                <FileSpreadsheet size={18} /> Import Mahasiswa dari CSV
              </h3>
            ...
            <form action={formAction} className="p-6 flex flex-col gap-4">
              ...
              {state.success && (
                (() => {
                  swal.fire({
                    ...
                    text: `${state.imported ?? 0} mahasiswa berhasil diimport.${state.skipped ? ` ${state.skipped} dilewati.` : ''}`,
                  })
                  state.success = false
                  return null
                })()
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={`Nama,NIM,Email,Semester,Kelas,NoHP\nAhmad Rizki,P2100001,ahmad@email.com,6,A,081234567890\nSiti Nurhaliza,P2100002,siti@email.com,6,B,`}
                />
              </div>
